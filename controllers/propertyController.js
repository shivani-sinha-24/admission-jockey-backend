...
export default {
    ...
    async createPropertyType(req, res) {
        ...
        try {
            ...
            let propertyExsist = await Property.findOne({ property_name: request.property_name })
            if (propertyExsist) {
                return res.status(200).json({ status_code: 300, "message": "This property already exsist!" })
            }
            let propertyCreated = await Property.create(request)
    ...
    async createGallery(req, res) {
        ...
        if (req?.files != undefined && req?.files?.gallery_img?.length > 0) {
            for (let i = 0; i < req?.files?.gallery_img?.length; i++) {
                gallery_img?.push('public/uploads/' + req?.files?.gallery_img[i]?.filename)
            }
    ...
    async createScholarship(req, res) {
        ...
        try {
            ...
            let exist = await Scholarship.findOne({ "title": request.title });
            ...
            return res.status(200).send({ status_code: 200, scholarship: scholarship, message: "Scholarship created successfully." });
    ...
    async createAdmission_process(req, res) {
        ...
        try {
            ...
            let exist = await Admission_process.findOne({ "title": request.title });
            ...
            return res.status(200).send({ status_code: 200, admission_process: admission_process, message: "Admission_process created successfully." });
    ...
    async createAnnouncement(req, res) {
        ...
        try {
            ...
            let exist = await Announcement.findOne({ "title": request.title });
            ...
            return res.status(200).send({ status_code: 200, announcement: announcement, message: "Announcement created successfully." });
